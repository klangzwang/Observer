...
#include "OBUILoadingSettings.generated.h"

class UObject;

UCLASS(config=Game, defaultconfig)
class UOBUILoadingSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:

	UOBUILoadingSettings();

public:
	
	UPROPERTY(config, EditAnywhere, Category = FadeInOut, meta = (MetaClass = "/Script/UMG.UserWidget"))
	FSoftClassPath FadeWidget;
	...
	// enabled via HoldLoadingScreenAdditionalSecsEvenInEditor
 	UPROPERTY(config, EditAnywhere, Category=Configuration, meta=(ForceUnits=s, ConsoleVariable="ObserverUMG.HoldLoadingScreenAdditionalSecs"))
	...
	UPROPERTY(Transient, EditAnywhere, Category=Debugging, meta=(ConsoleVariable="ObserverUMG.LogLoadingScreenReasonEveryFrame"))
	bool LogLoadingScreenReasonEveryFrame = 0;

	// Force the loading screen to be displayed (useful for debugging)
	UPROPERTY(Transient, EditAnywhere, Category=Debugging, meta=(ConsoleVariable="ObserverUMG.AlwaysShow"))
