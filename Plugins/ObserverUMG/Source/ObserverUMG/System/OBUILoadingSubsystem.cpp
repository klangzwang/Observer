...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OBUILoadingSubsystem)
...
bool IOBLoadingProcess::ShouldShowLoadingScreen(UObject* TestObject, FString& OutReason)
{
	if (TestObject != nullptr)
	{
		if (IOBLoadingProcess* LoadObserver = Cast<IOBLoadingProcess>(TestObject))
		{
...
namespace LoadingScreenCVars
{
	...
	static FAutoConsoleVariableRef CVarHoldLoadingScreenUpAtLeastThisLongInSecs(
		TEXT("ObserverUMG.HoldLoadingScreenAdditionalSecs"),
	...
	static FAutoConsoleVariableRef CVarLogLoadingScreenReasonEveryFrame(
		TEXT("ObserverUMG.LogLoadingScreenReasonEveryFrame"),
...
void UOBUILoadingSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
...
void UOBUILoadingSubsystem::Deinitialize()
{
	StopBlockingInput();
...
bool UOBUILoadingSubsystem::ShouldCreateSubsystem(UObject* Outer) const
{
...
void UOBUILoadingSubsystem::Tick(float DeltaTime)
{
	UpdateLoadingScreen();
...
ETickableTickType UOBUILoadingSubsystem::GetTickableTickType() const
{
	if (IsTemplate())
	{
...
bool UOBUILoadingSubsystem::IsTickable() const
{
...
TStatId UOBUILoadingSubsystem::GetStatId() const
{
	RETURN_QUICK_DECLARE_CYCLE_STAT(UOBUILoadingSubsystem, STATGROUP_Tickables);
}

UWorld* UOBUILoadingSubsystem::GetTickableGameObjectWorld() const
{
	return GetGameInstance()->GetWorld();
}

void UOBUILoadingSubsystem::RegisterLoadingProcessor(TScriptInterface<IOBLoadingProcess> Interface)
{
	ExternalLoadingProcessors.Add(Interface.GetObject());
}

void UOBUILoadingSubsystem::UnregisterLoadingProcessor(TScriptInterface<IOBLoadingProcess> Interface)
{
	ExternalLoadingProcessors.Remove(Interface.GetObject());
}

void UOBUILoadingSubsystem::HandlePreLoadMap(const FWorldContext& WorldContext, const FString& MapName)
{
...
void UOBUILoadingSubsystem::HandlePostLoadMap(UWorld* World)
{
...
void UOBUILoadingSubsystem::UpdateLoadingScreen()
{
	bool bLogLoadingScreenStatus = LoadingScreenCVars::LogLoadingScreenReasonEveryFrame;

	if (ShouldShowLoadingScreen())
	{
		const UOBUILoadingSettings* Settings = GetDefault<UOBUILoadingSettings>();
...
bool UOBUILoadingSubsystem::CheckForAnyNeedToShowLoadingScreen()
{
	...
	{
		DebugReasonForShowingOrHidingLoadingScreen = FString(TEXT("ObserverUMG.AlwaysShow is true"));
		return true;
	}
	...
	if (IOBLoadingProcess::ShouldShowLoadingScreen(GameState, /*out*/ DebugReasonForShowingOrHidingLoadingScreen))
	{
		return true;
	}
	...
	{
		if (IOBLoadingProcess::ShouldShowLoadingScreen(TestComponent, /*out*/ DebugReasonForShowingOrHidingLoadingScreen))
		{
			return true;
		}
	}
	...
	// streaming in.
	for (const TWeakInterfacePtr<IOBLoadingProcess>& Processor : ExternalLoadingProcessors)
	{
		if (IOBLoadingProcess::ShouldShowLoadingScreen(Processor.GetObject(), /*out*/ DebugReasonForShowingOrHidingLoadingScreen))
		{
			return true;
		}
	}
	...
	for (ULocalPlayer* LP : LocalGameInstance->GetLocalPlayers())
	{
		if (LP != nullptr)
		{
			if (APlayerController* PC = LP->PlayerController)
			{
				...
				if (IOBLoadingProcess::ShouldShowLoadingScreen(PC, /*out*/ DebugReasonForShowingOrHidingLoadingScreen))
				{
					return true;
				}
...
bool UOBUILoadingSubsystem::ShouldShowLoadingScreen()
{
	const UOBUILoadingSettings* Settings = GetDefault<UOBUILoadingSettings>();
...
bool UOBUILoadingSubsystem::IsShowingInitialLoadingScreen() const
{
...
void UOBUILoadingSubsystem::ShowLoadingScreen()
{
...
void UOBUILoadingSubsystem::HideLoadingScreen()
{
	if (!bCurrentlyShowingLoadingScreen)
	{
		return;
...
void UOBUILoadingSubsystem::RemoveWidgetFromViewport()
{
	UGameInstance* LocalGameInstance = GetGameInstance();
...
void UOBUILoadingSubsystem::StartBlockingInput()
{
...
void UOBUILoadingSubsystem::StopBlockingInput()
{
	if (InputPreProcessor.IsValid())
	{
...
void UOBUILoadingSubsystem::ChangePerformanceSettings(bool bEnabingLoadingScreen)
{
	UGameInstance* LocalGameInstance = GetGameInstance();
	UGameViewportClient* GameViewportClient = LocalGameInstance->GetGameViewportClient();
