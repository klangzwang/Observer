...
#include "OBPlayerStart.generated.h"

class AController;
class UObject;

enum class EOBPlayerStartLocationOccupancy
{
	Empty,
	Partial,
	Full
...
class AOBPlayerStart : public APlayerStart
{
	GENERATED_BODY()

public:

	AOBPlayerStart(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	const FGameplayTagContainer& GetGameplayTags() { return StartPointTags; }

	EOBPlayerStartLocationOccupancy GetLocationOccupancy(AController* const ControllerPawnToFit) const;
